...
EXPECT_TRUE(ex
.
Matches(std::string("abba")
));
EXPECT_EQ(2, ex.
Match(std::string("abba")
));

// match at start of string only!
EXPECT_FALSE(ex
.
Matches(std::string("baab")
));
EXPECT_EQ(-1, ex.
Match(std::string("baab")
));
}
...
EXPECT_TRUE(ex1
.
Matches(std::string("aaaaa")
));
EXPECT_EQ(4, ex1.
Match(std::string("aaaaa")
));
...
EXPECT_TRUE(ex
.
Matches(std::string("hello there")
));
EXPECT_FALSE(ex
.
Matches(std::string("hello ")
...
EXPECT_FALSE(ex
.
Matches(std::string("there")
));
EXPECT_EQ(11, ex.
Match(std::string("hello there")
));
}
