...
EXPECT_EQ("foo", node[1].

as<std::string>()

);
EXPECT_EQ("monkey", node[2].
...
EXPECT_EQ("monkey", node["undefined"].
...
EXPECT_EQ("test", node[0].

as<std::string>()

);
EXPECT_EQ("value", node[2].
...
EXPECT_EQ("v1", node["k1"].

as<std::string>()

);
EXPECT_EQ("v1", node["k2"].
...
std::size_t count = 0;
for (
const_iterator it = undefinedCn.begin();
it != undefinedCn.
...
std::size_t count = 0;
for (
iterator it = nonConstUndefinedNode.begin();
it != nonConstUndefinedNode.
...
EXPECT_EQ("12345", node["device"]["udid"].

as<std::string>()

);
EXPECT_EQ("iPhone", node["device"]["name"].

as<std::string>()

);
EXPECT_EQ("4.0", node["device"]["os"].

as<std::string>()

);
EXPECT_EQ("monkey", node["username"].
...
node["foo"] = "value";
node["bar"] = node["foo"];
EXPECT_EQ("value", node["foo"].

as<std::string>()

);
EXPECT_EQ("value", node["bar"].
...
Node node;
EXPECT_EQ(EmitterStyle::Default, node
.
