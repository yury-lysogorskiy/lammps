...
out <<
BeginMap;
out << Key << BeginSeq << 1 << 3 <<
EndSeq;
out << Value << "monster";
out << Key << Flow << BeginSeq << 2 << 0 <<
EndSeq;
out << Value << "demon";
out <<
EndMap;
...
out << BeginMap << Key << "key" << Value << "value" <<
EndMap;
...
out << Flow << BeginSeq << "foo" << Comment("foo!") << "bar" <<
EndSeq;
...
out <<
BeginMap;
out << Key << BeginSeq << 1 << 2 <<
EndSeq;
out << Value << BeginMap << Key << "a" << Value << "b" <<
EndMap;
out <<
EndMap;
out <<
...
out << Flow << BeginMap << Key << "C:" << Value << "C:" <<
EndMap;
ExpectEmit("{\"C:\": \"C:\"}");
}
