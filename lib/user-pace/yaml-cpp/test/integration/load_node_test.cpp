...
EXPECT_EQ(1.5f, node[0].

as<float>()

);
EXPECT_EQ(1.5, node[0].
...
EXPECT_EQ(std::numeric_limits<float>::infinity(), node[3]
...
std::numeric_limits<float>::infinity(), node[4]
...
EXPECT_FALSE(clone
== node);
EXPECT_EQ(NodeType::Sequence, clone
.
...
EXPECT_EQ(EmitterStyle::Flow, Load("[1, 2, 3]")
.

Style()

);
EXPECT_EQ(EmitterStyle::Flow, Load("{foo: bar}")
.
...
EXPECT_EQ(EmitterStyle::Block, Load("foo: bar")
.
...
for (
const ParserExceptionTestCase test
: tests) {
try {
Load(test
.input);
...
<< "Expected exception " << test.expected_exception << " for "
<< test.name << ", input: " << test.
input;
} catch (
const ParserException &e
) {
EXPECT_EQ(test
.expected_exception, e.msg);
}
}
}
