...
run_test() {
    ...
    # Create temporary input file with warmup + measurement runs
...
# Warmup run (model compilation)
fix 1 all nvt temp 300.0 300.0 0.1
run 2
unfix 1

# Reset timers and do measurement run
fix 2 all nvt temp 300.0 300.0 0.1
run 3
EOF

    # Run with proper GPU binding
    if [ "$np" -eq 1 ]; then
        CUDA_VISIBLE_DEVICES=0 $LMP -in temp_scaling.in 2>&1 | grep -E "\[GRACE-PROFILE\]|Loop time|atoms" | tail -6 >> $RESULTS
    else
        mpirun -np $np --bind-to none bash -c 'CUDA_VISIBLE_DEVICES=$((OMPI_COMM_WORLD_RANK % 4)) '"$LMP"' -in temp_scaling.in' 2>&1 | grep -E "\[GRACE-PROFILE\]|Loop time|atoms" | tail -6 >> $RESULTS
    fi
    echo "" >> $RESULTS
}
...
run_test 2 "grace/2layer/parallel" 10 "parallel-2np-2000at-weak"  # 2000 atoms (~1000/rank)
run_test 4 "grace/2layer/parallel" 13 "parallel-4np-4394at-weak"  # ~4394 atoms (~1100/rank)
