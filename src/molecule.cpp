...
void Molecule::from_json(const std::string &molid, const json &moldata)
{
  ...
  if (moldata.contains("application")) {
    if (moldata["application"] != "LAMMPS")
      error->all(FLERR, Error::NOLASTLINE,
                 ...
                 moldata["application"].get<std::string>());
  } else {
    error->all(FLERR, Error::NOLASTLINE,
  ...
  if (moldata.contains("units")) {
    ...
    auto jsonunits = moldata["units"].get<std::string>();
  ...
  if (moldata.contains("title")) title = moldata["title"].get<std::string>();
...
#undef JSON_INIT_FIELD
  ...
  if (moldata.contains("masstotal")) {
    ...
    masstotal = moldata["masstotal"].get<double>() * sizescale * sizescale * sizescale;
  }

  if (moldata.contains("com") && (moldata["com"].size() == 3)) {
    comflag = comflag_user = 1;
    com[0] = moldata["com"][0].get<double>() * sizescale;
    com[1] = moldata["com"][1].get<double>() * sizescale;
    com[2] = moldata["com"][2].get<double>() * sizescale;
  }

  if (moldata.contains("inertia") && (moldata["inertia"].size() == 6)) {
    inertiaflag = inertiaflag_user = 1;
    const double scale5 = powint(sizescale, 5);
    itensor[0] = moldata["inertia"][0].get<double>() * scale5;
    itensor[1] = moldata["inertia"][1].get<double>() * scale5;
    itensor[2] = moldata["inertia"][2].get<double>() * scale5;
    itensor[3] = moldata["inertia"][3].get<double>() * scale5;
    itensor[4] = moldata["inertia"][4].get<double>() * scale5;
    itensor[5] = moldata["inertia"][5].get<double>() * scale5;
  }
  ...
  // coords
  if (xflag) {
    for (int i = 0; i < 4; ++i) secfmt.emplace_back(moldata["coords"]["format"][i].get<std::string>());
      ...
      for (const auto &c : moldata["coords"]["data"]) {
        ...
        if ((iatom < 0) || (iatom >= natoms))
          error->all(
              ...
              id, iatom + 1);
        count[iatom]++;
        x[iatom][0] = c[1].get<double>();
        x[iatom][1] = c[2].get<double>();
        x[iatom][2] = c[3].get<double>();
  ...
  secfmt.clear();
  for (int i = 0; i < 2; ++i) secfmt.emplace_back(moldata["types"]["format"][i].get<std::string>());
  if ((secfmt[0] == "atom-id") && (secfmt[1] == "type")) {
    ...
    for (const auto &c : moldata["types"]["data"]) {
      ...
      if (c[1].is_number_integer()) {    // numeric type
        type[iatom] = c[1].get<int>() + toffset;
      } else {
        const auto &typestr = c[1].get<std::string>();
        if (!atom->labelmapflag)
          error->all(FLERR, Error::NOLASTLINE,
  ...
  if (moleculeflag) {
    ...
    secfmt.clear();
    for (int i = 0; i < 2; ++i) secfmt.emplace_back(moldata["molecules"]["format"][i].get<std::string>());
  ...
  if (fragmentflag) {
    secfmt.clear();
    for (int i = 0; i < 2; ++i) secfmt.emplace_back(moldata["fragments"]["format"][i].get<std::string>());
  ...
  if (qflag) {
    ...
    secfmt.clear();
    for (int i = 0; i < 2; ++i) secfmt.emplace_back(moldata["charges"]["format"][i].get<std::string>());
  ...
  if (radiusflag && !bodyflag) {
    maxradius = 0.0;
    secfmt.clear();
    for (int i = 0; i < 2; ++i) secfmt.emplace_back(moldata["diameters"]["format"][i].get<std::string>());
      ...
      for (const auto &c : moldata["diameters"]["data"]) {
        ...
        if (!c[1].is_number())
          error->all(FLERR, Error::NOLASTLINE,
                     ...
                     "molecule JSON data: {}",
                     id, to_string(c[1]));
        radius[iatom] = c[1].get<double>() * sizescale * 0.5;
  ...
  if (muflag) {
    ...
    secfmt.clear();
    for (int i = 0; i < 4; ++i) secfmt.emplace_back(moldata["dipoles"]["format"][i].get<std::string>());
      ...
      for (const auto &c : moldata["dipoles"]["data"]) {
        ...
        if ((iatom < 0) || (iatom >= natoms))
          error->all(
              ...
              id, iatom + 1);
        count[iatom]++;
        mu[iatom][0] = c[1].get<double>();
        mu[iatom][1] = c[2].get<double>();
        mu[iatom][2] = c[3].get<double>();
  ...
  if (rmassflag) {
    secfmt.clear();
    for (int i = 0; i < 2; ++i) secfmt.emplace_back(moldata["masses"]["format"][i].get<std::string>());
      ...
      for (const auto &c : moldata["masses"]["data"]) {
        ...
        if (!c[1].is_number())
          error->all(FLERR, Error::NOLASTLINE,
                     ...
                     "molecule JSON data: {}",
                     id, to_string(c[1]));
        rmass[iatom] = c[1].get<double>() * sizescale * sizescale * sizescale;
        count[iatom]++;
      }
      // checks
      for (int i = 0; i < natoms; i++) {
  ...
  if (bondflag) {
    ...
    for (int flag = 0; flag < 2; ++flag) {
      secfmt.clear();
      for (int i = 0; i < 3; ++i) secfmt.emplace_back(moldata["bonds"]["format"][i].get<std::string>());
      if ((secfmt[0] == "bond-type") && (secfmt[1] == "atom1") && (secfmt[2] == "atom2")) {
        ...
        for (int i = 0; i < nbonds; ++i) {
          ...
          if (item[0].is_number_integer()) {    // numeric type
            itype = item[0].get<int>() + boffset;
          } else {
            const auto &typestr = item[0].get<std::string>();
            if (!atom->labelmapflag)
              error->all(FLERR, Error::NOLASTLINE,
          ...
          atom1 = item[1].get<tagint>();
          atom2 = item[2].get<tagint>();
  ...
  if (angleflag) {
    ...
    for (int flag = 0; flag < 2; ++flag) {
      secfmt.clear();
      for (int i = 0; i < 4; ++i) secfmt.emplace_back(moldata["angles"]["format"][i].get<std::string>());
  ...
  if (dihedralflag) {
    ...
    for (int flag = 0; flag < 2; ++flag) {
      secfmt.clear();
      for (int i = 0; i < 5; ++i) secfmt.emplace_back(moldata["dihedrals"]["format"][i].get<std::string>());
  ...
  if (improperflag) {
    ...
    for (int flag = 0; flag < 2; ++flag) {
      secfmt.clear();
      for (int i = 0; i < 5; ++i) secfmt.emplace_back(moldata["impropers"]["format"][i].get<std::string>());
  ...
  if (specialflag) {
    ...
    const auto &specialcounts = moldata["special"]["counts"];
    secfmt.clear();
    for (int i = 0; i < 4; ++i) secfmt.emplace_back(specialcounts["format"][i].get<std::string>());
    ...
    const auto &specialbonds = moldata["special"]["bonds"];
    secfmt.clear();
    for (int i = 0; i < 2; ++i) secfmt.emplace_back(specialbonds["format"][i].get<std::string>());
    if ((secfmt[0] == "atom-id") && (secfmt[1] == "atom-id-list")) {
      ...
      for (int i = 0; i < natoms; ++i) {
        ...
        const int iatom = specialbonds["data"][i][0].get<int>() - 1;
        if ((iatom < 0) || (iatom >= natoms))
          error->all(FLERR, Error::NOLASTLINE,
  ...
  if (shakeflagflag) {
    ...
    secfmt.clear();
    for (int i = 0; i < 2; ++i) secfmt.emplace_back(shakedata["format"][i].get<std::string>());
...
#define APPLY_SHAKE_ATOMS(ncols)                                                          \
  ...
  for (int i = 0; i < ncols; ++i) {                                                       \
    ...
    shake_atom[iatom][i] = c[1][i].get<int>();                                                  \
  }
...
#define SET_SHAKE_TYPE(type, idx, ncols, offset)                                          \
  ...
  if (c[1][idx].is_number_integer()) {                                                    \
    shake_type[iatom][idx] = c[1][idx].get<int>() + offset;                                     \
  } else {                                                                                \
    char *subst = utils::expand_type(FLERR, c[1][idx].get<std::string>(), type, lmp);                        \
    if (subst) {                                                                          \
...
#undef SET_SHAKE_TYPE
  ...
  if (bodyflag) {
    for (int i = 0; i < nibody; ++i) ibodyparams[i] = moldata["body"]["integers"][i].get<int>();
    for (int i = 0; i < ndbody; ++i) dbodyparams[i] = moldata["body"]["doubles"][i].get<double>();
  }
