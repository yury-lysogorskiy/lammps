...
void GRACE1LCPUModel::load(const std::string &filepath) {
  ...
  if (uq_schema_version != 0) {
    ...
    if (uq_schema_version != uq_schema_supported)
      throw std::runtime_error("GRACE-1L/CPU/KK: unsupported uq_schema_version " +
...
void PairGRACE1LCPUKokkos<DeviceType, NNScalarT, GeomScalarT>::coeff(int narg, char **arg)
{
  ...
  if (has_uq && comm->me == 0)
    utils::logmesg(lmp, "[GRACE-1L/CPU/KK] UQ artifacts loaded: Kmax={}, D={}, error_model={}\n",
...
void PairGRACE1LCPUKokkos<DeviceType, NNScalarT, GeomScalarT>::copy_weights_to_device()
{
  ...
  if (m.has_uq) {
    ...
    if (D != 1 + nin_last)
      error->all(FLERR, "GRACE-1L/CPU/KK UQ: uq_feature_dim ({}) != 1 + energy-MLP last "
                        "hidden width ({})", D, nin_last);
    if (E != m.n_elements)
      error->all(FLERR, "GRACE-1L/CPU/KK UQ: uq_n_elements ({}) != model n_elements ({})",
                 E, m.n_elements);
    if (D > GRACE1L_CPU_MAX_MLP_DIM + 1)
      error->all(FLERR, "GRACE-1L/CPU/KK UQ: feature_dim ({}) exceeds GRACE1L_CPU_MAX_MLP_DIM+1 ({})",
                 D, GRACE1L_CPU_MAX_MLP_DIM + 1);
...
// energy hot path is untouched (no-UQ run pays nothing).
// f = [lin_origin = rho[0], hidden = penultimate-MLP-layer output], dim D.
// ======================================================================
...
void PairGRACE1LCPUKokkos<DeviceType, NNScalarT, GeomScalarT>::operator() (TagComputeUQ, const int& ii) const
{
  ...
  const int D = uq_D;
  double f[GRACE1L_CPU_MAX_MLP_DIM + 1];
  ...
  double delta[GRACE1L_CPU_MAX_MLP_DIM + 1];
