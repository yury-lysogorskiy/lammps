...
PairPACETensorPotentialCellKokkos<DeviceType>::PairPACETensorPotentialCellKokkos(LAMMPS *lmp)
        : PairPACETensorPotentialCell(lmp) {
    respa_enable = 0;
...
//    }
    ...
    auto d_actual_jnum  = Kokkos::View<int *, DeviceType>("d_actual_jnum", nlocal); // grow ?
    ...
    printf("LABEL 3\n");
//    printf("d_actual_jnum_shift.extent=%ld\n",d_actual_jnum_shift.extent(0));

    printf("h_actual_jnum=");
    auto h_actual_jnum = Kokkos::create_mirror_view(d_actual_jnum);
    Kokkos::deep_copy(h_actual_jnum, d_actual_jnum);
    for (int i = 0; i < nlocal; i++)
        printf("%d ", h_actual_jnum(i));
    printf("\n");



    // count number of real neighbours and build shift vector d_actual_jnum_shift
    auto d_actual_jnum_shift = Kokkos::View<int *, DeviceType>("d_actual_jnum_shift", nlocal); // grow ?
    {
        n_real_neighbours = 0;
    ...
    printf("h_actual_jnum_shift=");
    auto h_actual_jnum_shift = Kokkos::create_mirror_view(d_actual_jnum_shift);
    Kokkos::deep_copy(h_actual_jnum_shift, d_actual_jnum_shift);
    for (int i = 0; i < nlocal; i++)
        printf("%d ", h_actual_jnum_shift(i));
    printf("\n");
    ...
    auto d_ind_i_vector = Kokkos::View<int32_t*, DeviceType>("d_ind_i_vector",tot_neighbours); //tot_neighbours);
    auto d_ind_j_vector = Kokkos::View<int32_t*, DeviceType>("d_ind_j_vector", tot_neighbours);
    auto d_mu_i_vector = Kokkos::View<int32_t*, DeviceType>("d_mu_i_vector", tot_neighbours);
    auto d_mu_j_vector = Kokkos::View<int32_t*, DeviceType>("d_mu_j_vector", tot_neighbours);
    auto d_vector_offset = Kokkos::View<double *[3], DeviceType>("d_vector_offset", tot_neighbours, 3);
    printf("LABEL 6\n");
    auto d_atomic_mu_i_vector = Kokkos::View<int32_t*, DeviceType>("d_atomic_mu_i_vector",nlocal+n_fake_atoms);
    auto h_atomic_mu_i_vector0 = Kokkos::create_mirror_view(d_atomic_mu_i_vector);
    Kokkos::deep_copy(h_atomic_mu_i_vector0, d_atomic_mu_i_vector);
    printf("h_atomic_mu_i_vector0(before)= ");
    for(int i=0;i<nlocal;++i) printf("%d ",h_atomic_mu_i_vector0(i));
    printf("\n");
    ...
    Kokkos::parallel_for("PairPACETensorPotentialCellKokkos:d_atomic_mu_i_vector", nlocal, KOKKOS_LAMBDA(const int ii) {
        const int i = d_ilist(ii);
        d_atomic_mu_i_vector(ii) = 0; // d_map(type(i));
    });

    auto h_atomic_mu_i_vector1 = Kokkos::create_mirror_view(d_atomic_mu_i_vector);
    Kokkos::deep_copy(h_atomic_mu_i_vector1, d_atomic_mu_i_vector);
    printf("h_atomic_mu_i_vector1= ");
    for(int i=0;i<nlocal;++i) printf("%d ",h_atomic_mu_i_vector1(i));
    printf("\n");

//    cppflow::tensor atomic_mu_i_vector_tens1(TF_INT32, d_atomic_mu_i_vector.data(),
//                                            (nlocal + n_fake_atoms) * sizeof(int), {nlocal + n_fake_atoms});
    ...
    {
        Kokkos::parallel_for("PairPACETensorPotentialCellKokkos: Fill bond-related vectors", nlocal, KOKKOS_LAMBDA(
        const int ii) {
            const int i = d_ilist(ii);
            const double xtmp = x(i, 0);
            const double ytmp = x(i, 1);
            ...
            for (int jj = 0; jj < d_numneigh(i); ++jj) {
                ...
                if (rsq < cutoff_sq) {
                    ...
                    d_mu_i_vector(tot_ind) = 0; //d_map(type(i));
                    d_mu_j_vector(tot_ind) = 0; //d_map(type(j));
        ...
        Kokkos::parallel_for("PairPACETensorPotentialCellKokkos: Fake fill bond-related vectors", n_fake_neighbours,
    ...
    auto d_positions = Kokkos::View<double *[3], DeviceType>("d_vector_offset", nlocal+n_fake_atoms, 3);
    printf("LABEL 8\n");
    {
        Kokkos::parallel_for("PairMACEKokkos: Fill d_positions.", nlocal, KOKKOS_LAMBDA(
        const int i) {
    ...
    cppflow::tensor atomic_mu_i_vector_tens(TF_INT32, (void*)h_atomic_mu_i_vector.data(),
                                            (nlocal + n_fake_atoms) * sizeof(int), {nlocal + n_fake_atoms});
    inputs.emplace_back(DEFAULT_INPUT_PREFIX + "atomic_mu_i" + ":0", atomic_mu_i_vector_tens);
    std::cout << "atomic_mu_i_vector_tens: " << atomic_mu_i_vector_tens << std::endl;
    ...
    cppflow::tensor vector_offset_tens(TF_DOUBLE, h_vector_offset.data(), tot_neighbours * 3 * sizeof(double),
                                       {tot_neighbours, 3});
    inputs.emplace_back(DEFAULT_INPUT_PREFIX + "vector_offsets" + ":0", vector_offset_tens);
    std::cout << "vector_offset_tens: " << vector_offset_tens << std::endl;
    ...
    std::cout << "pos_tens: " << pos_tens << std::endl;
    ...
    if (eflag_global) {
        ...
        std::cout << "e_tens: " <<  output[0] << std::endl;
        ...
        auto d_atom_energy = Kokkos::View<double *, DeviceType>("d_atom_energy", nlocal + n_fake_atoms);
        Kokkos::deep_copy(d_atom_energy, h_atom_energy);
        printf("h_atom_energy= ");
        for(int i=0;i<nlocal;++i) printf("%f ",h_atom_energy(i));
        printf("\n");
...
//            const int i = d_ilist(ii);
            eng_vdwl += d_atom_energy(ii);
        }, eng_vdwl);
    }

    auto &f_out = output[1];
    auto f_tens = f_out.get_tensor();
    std::cout << "f_tens: " <<  output[1] << std::endl;
...
namespace LAMMPS_NS {
    template
    class PairPACETensorPotentialCellKokkos<LMPDeviceType>;
